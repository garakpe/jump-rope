...
class ReflectionProvider extends ChangeNotifier {
  ...
  Future<void> _loadReflectionQuestions() async {
    ...
    try {
      ...
      _reflectionCards = List.from(reflectionCards); // 직접 참조 대신 복사본 사용
      _isLoading = false;
      notifyListeners();
    } catch (e) {
      _isLoading = false;
  ...
  Future<void> submitReflection(ReflectionSubmission submission) async {
    ...
    try {
      // 선택된 ReflectionModel 찾기
      final selectedReflection = _reflectionCards.firstWhere(
          (r) => r.id == submission.reflectionId,
          orElse: () => throw Exception("선택된 성찰 양식을 찾을 수 없습니다"));
  ...
  Future<ReflectionSubmission?> getSubmission(
    ...
    try {
      ...
      if (reflection == null) {
        // 성찰 질문 모델 찾기
        final reflectionModel = _reflectionCards.firstWhere(
            (r) => r.id == reflectionId,
            orElse: () => throw Exception("성찰 양식을 찾을 수 없습니다"));

        // 빈 제출 객체 생성
        return ReflectionSubmission(
          studentId: studentId,
          reflectionId: reflectionId,
          week: reflectionModel.week,
          answers: {},
